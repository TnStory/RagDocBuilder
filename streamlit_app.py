...
def call_url_build(url):
    params = {'url': url}
    response = requests.get("http://localhost:8080/api/rag/url_build", params=params)
...
def call_file_build(file):
    ...
    response = requests.post("http://localhost:8080/api/rag/file_build", files=files)
    if response.status_code == 200:
        return response.json()  # 응답을 JSON 형식으로 가정
...
def call_build_list(build_type=None, page=1, page_size=10):
    ...
    response = requests.get("http://localhost:8080/api/rag/build_list", params=params)
    if response.status_code == 200:
...
def call_similarity_search(key_word):
    ...
    response = requests.post("http://localhost:8080/api/rag/similarity_search", json=json_data)
    if response.status_code == 200:
        return response.json()  # 응답을 JSON 형식으로 가정
